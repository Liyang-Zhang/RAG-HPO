...
@click.group()
def cli():
    """RAG-HPO: Retrieval-Augmented Generation for HPO annotation."""
    pass
...
@click.option(
    ...
    default=14,
    type=int,
    help="Refresh OBO file if older than this many days.",
)
@click.option("--meta-output", default="hpo_meta.json", help="Output path for HPO metadata JSON.")
@click.option(
    "--vec-output",
    default="hpo_embedded.npz",
    help="Output path for HPO embeddings NPZ.",
)
@click.option(
    "--hpo-full-csv",
    default="hpo_terms_full.csv",
    help="Output path for full HPO terms CSV.",
)
@click.option(
    "--chpo-path",
    type=click.Path(exists=True),
    help="Optional path to CHPO Excel/CSV with Chinese translations.",
)
@click.option(
    "--use-sbert/--no-sbert",
    default=True,
    help="Use SBERT for embeddings (default: True).",
)
@click.option(
    "--sbert-model",
    default="pritamdeka/SapBERT-mnli-snli-scinli-scitail-mednli-stsb",
...
@click.option(
    ...
    type=click.Path(exists=True),
    required=True,
    help="Path to input CSV file with clinical notes.",
)
@click.option(
    "--output-csv",
    type=click.Path(),
    help="Path to save the output CSV with HPO annotations.",
)
@click.option(
    "--output-json-raw",
    type=click.Path(),
    help="Path to save the raw JSON output from LLM.",
)
...
@click.option(
    "--api-key",
    envvar="LLM_API_KEY",
    help="API key for the LLM. Can be set via LLM_API_KEY environment variable.",
)
@click.option(
    "--base-url",
    envvar="LLM_BASE_URL",
    default="https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions",
    help="Base URL for the LLM API. Can be set via LLM_BASE_URL environment variable.",
)
@click.option(
    "--llm-model-name",
    envvar="LLM_MODEL_NAME",
    default="qwen3-max",
    help="LLM model name. Can be set via LLM_MODEL_NAME environment variable.",
)
...
@click.option(
    ...
    default=30,
    type=int,
    help="Max queries per minute for LLM.",
)
